function submitBoard(){    
    ...
    //사용자가 입력한 값을 서버에 전송해 web_board 테이블에 저장되도록 AJAX 코드 작성
    ...
    //json형식으로 보내보기..(요즘 선호 방식 -- 요청 응답 모두 json형식)

    //json을 형식의 문자열을 곧바로 만드는 것은 불편. 그래서 JS 객체로 생성
    var data={
        title:title, //멤버변수명: 값
        writer:writer,
        password:password,
    ...
    fetch('../backend/board/insertBoard.php',{
        method:'POST',
        headers:{'Content-Type':'application/json'}, //보내는 데이터가 json임을 알려줌
        body:jsonData
    })
    .then(function(res){
        return res.text();
